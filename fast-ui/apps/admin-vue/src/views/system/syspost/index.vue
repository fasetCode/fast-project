    ...
    <div class="filter-bar">
      <a-form :model="queryParams" class="elegant-form">
        <div class="filter-grid">
          ...
          <div class="filter-item">
            <span class="filter-label">岗位编码</span>
            <a-input 
      ...
      <div class="board-toolbar">
        <div class="toolbar-left">
          <div class="board-tabs">
            <div class="tab active">全部岗位 <span class="badge">{{ pagination.total }}</span></div>
          </div>
        </div>
      ...
      <div class="modal-inner">
        <div class="modal-header">
          ...
          <div class="modal-titles">
            <h2>{{ isEdit ? '编辑岗位信息' : '创建新岗位' }}</h2>
            <p>请填写以下必填信息以完成岗位设置</p>
          </div>
        </div>
          ...
          <a-row :gutter="24">
            <a-col :span="12">
              <a-form-item name="name">
                <template #label><span class="form-label">岗位名称</span></template>
                <a-input v-model:value="formData.name" placeholder="如：项目经理" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item name="code">
                <template #label><span class="form-label">岗位编码</span></template>
                <a-input v-model:value="formData.code" placeholder="如：manager" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
            ...
            <a-col :span="12">
              <a-form-item name="status">
                <template #label><span class="form-label">岗位状态</span></template>
                <StatusCardSelect 
...
import type { SysPostVo, SysPostQuery, SysPostCreate, SysPostUpdate } from '@/api/system/syspost.ts'
import { getPostPage, createPost, updatePost, deletePost, batchDeletePost, getPostById } from '@/api/system/syspost.ts'
...
const loadButtonPermissions = async () => {
  ...
  canAdd.value = hasButtonPermission('admin:system:post:add')
  canUpdate.value = hasButtonPermission('admin:system:post:update')
  canDelete.value = hasButtonPermission('admin:system:post:delete')
}
...
const statusOptions = computed(() => {
  ...
  return statusData.map((item: any) => ({
    ...
    title: item.label,
    // desc: item.value === '1' ? '岗位可用，可正常分配人员' : '岗位不可用，无法分配人员',
...
const columns = [
  ...
  { title: '岗位编码', dataIndex: 'code', key: 'code', width: 150 },
...
type SysPostFormData = SysPostCreate & { id?: number }
const formData = reactive<SysPostFormData>({
  id: undefined,
  name: '',
  code: '',
  sort: 0,
...
const handleAdd = () => {
  ...
  Object.assign(formData, {
    id: undefined, name: '', code: '', sort: 0, status: 0, remark: '',
  })
  modalVisible.value = true
}

const handleEdit = async (record: SysPostVo) => {
  isEdit.value = true
  try {
    const res = await getPostById(record.id!)
    if (res.code === 200) {
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) {
      const res = await updatePost(formData as SysPostUpdate, requestId.value)
      if (res.code === 200) {
        message.success({ content: '岗位更新成功', class: 'custom-message' })
        modalVisible.value = false
        loadData()
      }
    } else {
      const submitData: SysPostCreate = { ...(formData as any) }
      delete (submitData as any).id
      const res = await createPost(submitData, requestId.value)
      if (res.code === 200) {
        message.success({ content: '岗位创建成功', class: 'custom-message' })
        modalVisible.value = false
        loadData()
      }
    }
...
const handleDeleteOne = (record: SysPostVo) => {
  Modal.confirm({
    title: '危险操作确认',
    content: `将永久删除岗位「${record.name}」，此操作无法撤销。`,
    okText: '确认删除',
    okType: 'danger',
    cancelText: '取消',
    centered: true,
...
const handleDelete = () => {
  ...
  Modal.confirm({
    title: '批量删除确认',
    content: `将彻底删除选中的 ${selectedRowKeys.value.length} 个岗位，确定继续？`,
    okText: '全部删除',
    okType: 'danger',
    cancelText: '取消',
    centered: true,
    class: 'danger-modal',
    onOk: async () => {
      const res = await batchDeletePost(selectedRowKeys.value)
      if (res.code === 200) {
...
const handleStatusChange = async (record: SysPostVo) => {
  const newStatus = record.status === 1 ? 2 : 1
  requestId.value = getRequestId();
  const res = await updatePost({ ...record, status: newStatus } as SysPostUpdate, requestId.value)
  if (res.code === 200) {
    record.status = newStatus
    message.success(newStatus === 1 ? '岗位已启用' : '岗位已禁用')
  }
}
