      ...
      <div class="board-toolbar">
        <div class="toolbar-left">
          <div class="board-tabs">
            <div class="tab active">浏览记录 <span class="badge">{{ pagination.total }}</span></div>
          </div>
        </div>
    ...
    <a-modal v-model:open="modalVisible" :title="isEdit ? '编辑记录' : '新增记录'" width="900px" :destroy-on-close="true" @cancel="handleCancel" @ok="handleSubmit" :confirm-loading="submitLoading">
      <a-form ref="formRef" :model="formData" layout="vertical" :rules="rules">
        <a-row :gutter="16">
          ...
          <a-col :span="8"><a-form-item label="IP" name="ip"><a-input v-model:value="formData.ip" /></a-form-item></a-col>
          <a-col :span="24"><a-form-item label="User-Agent" name="userAgent"><a-textarea v-model:value="formData.userAgent" :rows="2" /></a-form-item></a-col>
          <a-col :span="24"><a-form-item label="Referer" name="referer"><a-textarea v-model:value="formData.referer" :rows="2" /></a-form-item></a-col>
        </a-row>
      </a-form>
    </a-modal>
  </div>
...
import type { ContentViewLogVo, ContentViewLogQuery, ContentViewLogCreate, ContentViewLogUpdate } from '@/api/content/contentviewlog'
import { getContentViewLogPage, getContentViewLogById, createContentViewLog, updateContentViewLog, deleteContentViewLog, batchDeleteContentViewLog } from '@/api/content/contentviewlog'
...
const columns = [
  ...
  { title: 'Referer', dataIndex: 'referer', key: 'referer', width: 240, ellipsis: true },
  { title: '操作', key: 'action', width: 160, fixed: 'right' as const, align: 'center' as const },
]

const loading = ref(false)
const submitLoading = ref(false)
const dataSource = ref<ContentViewLogVo[]>([])
const selectedRowKeys = ref<number[]>([])

const queryParams = reactive<ContentViewLogQuery>({
  page: 1,
  pageSize: 10,
})
...
type ContentViewLogFormData = ContentViewLogCreate & { id?: number }
const formData = reactive<ContentViewLogFormData>({
  id: undefined,
  contentId: undefined,
  userId: undefined,
  ip: '',
...
const loadData = async () => {
  loading.value = true
  try {
    const params: ContentViewLogQuery = { ...queryParams, page: pagination.current - 1, pageSize: pagination.pageSize }
    const res: any = await getContentViewLogPage(params)
    if (res.code === 200) {
...
const handleAdd = () => {
  isEdit.value = false
  Object.assign(formData, { id: undefined, contentId: undefined, userId: undefined, ip: '', userAgent: '', referer: '' })
  modalVisible.value = true
}

const handleEdit = async (record: ContentViewLogVo) => {
  isEdit.value = true
  const res: any = await getContentViewLogById(record.id!)
  if (res.code === 200) {
...
const handleSubmit = async () => {
  try {
    ...
    const res: any = isEdit.value
      ? await updateContentViewLog(payload as ContentViewLogUpdate, reqId)
      : await createContentViewLog(payload, reqId)
    if (res.code === 200) {
...
const handleDeleteOne = (record: ContentViewLogVo) => {
  Modal.confirm({
    title: '确认删除',
    ...
    centered: true,
    onOk: async () => {
      const res: any = await deleteContentViewLog(record.id!)
      if (res.code === 200) {
...
const handleDelete = () => {
  ...
  Modal.confirm({
    ...
    centered: true,
    onOk: async () => {
      const res: any = await batchDeleteContentViewLog(selectedRowKeys.value)
      if (res.code === 200) {
        message.success('删除成功')
