      ...
      <div class="board-toolbar">
        <div class="toolbar-left">
          <div class="board-tabs">
            <div class="tab active">点赞记录 <span class="badge">{{ pagination.total }}</span></div>
          </div>
        </div>
...
import type { ContentLikeVo, ContentLikeQuery, ContentLikeCreate, ContentLikeUpdate } from '@/api/content/contentlike'
import { getContentLikePage, getContentLikeById, createContentLike, updateContentLike, deleteContentLike, batchDeleteContentLike } from '@/api/content/contentlike'
...
const dataSource = ref<ContentLikeVo[]>([])
const selectedRowKeys = ref<number[]>([])

const queryParams = reactive<ContentLikeQuery>({
  page: 1,
  pageSize: 10,
})
...
type ContentLikeFormData = ContentLikeCreate & { id?: number }
const formData = reactive<ContentLikeFormData>({
  id: undefined,
  contentId: undefined,
  userId: undefined,
})
...
const loadData = async () => {
  loading.value = true
  try {
    const params: ContentLikeQuery = { ...queryParams, page: pagination.current - 1, pageSize: pagination.pageSize }
    const res: any = await getContentLikePage(params)
    if (res.code === 200) {
...
const handleEdit = async (record: ContentLikeVo) => {
  isEdit.value = true
  const res: any = await getContentLikeById(record.id!)
  if (res.code === 200) {
...
const handleSubmit = async () => {
  try {
    ...
    const res: any = isEdit.value
      ? await updateContentLike(payload as ContentLikeUpdate, reqId)
      : await createContentLike(payload, reqId)
    if (res.code === 200) {
...
const handleDeleteOne = (record: ContentLikeVo) => {
  Modal.confirm({
    title: '确认删除',
    ...
    centered: true,
    onOk: async () => {
      const res: any = await deleteContentLike(record.id!)
      if (res.code === 200) {
...
const handleDelete = () => {
  ...
  Modal.confirm({
    ...
    centered: true,
    onOk: async () => {
      const res: any = await batchDeleteContentLike(selectedRowKeys.value)
      if (res.code === 200) {
        message.success('删除成功')
