<template>
  <div class="mall-container">
      ...
      <div class="board-toolbar">
        <div class="toolbar-left">
          <div class="board-tabs">
            <div class="tab active">收藏记录 <span class="badge">{{ pagination.total }}</span></div>
          </div>
        </div>
    ...
    <a-modal v-model:open="modalVisible" :title="isEdit ? '编辑记录' : '新增记录'" width="520px" :destroy-on-close="true" @cancel="handleCancel" @ok="handleSubmit" :confirm-loading="submitLoading">
...
import type { ContentFavoriteVo, ContentFavoriteQuery, ContentFavoriteCreate, ContentFavoriteUpdate } from '@/api/content/contentfavorite'
import { getContentFavoritePage, getContentFavoriteById, createContentFavorite, updateContentFavorite, deleteContentFavorite, batchDeleteContentFavorite } from '@/api/content/contentfavorite'
import { getRequestId } from '@/utils/idUtils.ts'

const columns = [
  { title: 'ID', dataIndex: 'id', key: 'id', width: 90 },
  { title: '内容ID', dataIndex: 'contentId', key: 'contentId', width: 200 },
...
const dataSource = ref<ContentFavoriteVo[]>([])
const selectedRowKeys = ref<number[]>([])

const queryParams = reactive<ContentFavoriteQuery>({
  page: 1,
  pageSize: 10,
})
...
type ContentFavoriteFormData = ContentFavoriteCreate & { id?: number }
const formData = reactive<ContentFavoriteFormData>({
  id: undefined,
  contentId: undefined,
  userId: undefined,
})
...
const loadData = async () => {
  loading.value = true
  try {
    const params: ContentFavoriteQuery = { ...queryParams, page: pagination.current - 1, pageSize: pagination.pageSize }
    const res: any = await getContentFavoritePage(params)
    if (res.code === 200) {
...
const handleAdd = () => {
  isEdit.value = false
  Object.assign(formData, { id: undefined, contentId: undefined, userId: undefined })
  modalVisible.value = true
}

const handleEdit = async (record: ContentFavoriteVo) => {
  isEdit.value = true
  const res: any = await getContentFavoriteById(record.id!)
  if (res.code === 200) {
...
const handleSubmit = async () => {
  try {
    ...
    const res: any = isEdit.value
      ? await updateContentFavorite(payload as ContentFavoriteUpdate, reqId)
      : await createContentFavorite(payload, reqId)
    if (res.code === 200) {
...
const handleDeleteOne = (record: ContentFavoriteVo) => {
  Modal.confirm({
    title: '确认删除',
    ...
    centered: true,
    onOk: async () => {
      const res: any = await deleteContentFavorite(record.id!)
      if (res.code === 200) {
...
const handleDelete = () => {
  ...
  Modal.confirm({
    ...
    centered: true,
    onOk: async () => {
      const res: any = await batchDeleteContentFavorite(selectedRowKeys.value)
      if (res.code === 200) {
        message.success('删除成功')
