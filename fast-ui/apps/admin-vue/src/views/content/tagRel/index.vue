      ...
      <div class="board-toolbar">
        <div class="toolbar-left">
          <div class="board-tabs">
            <div class="tab active">标签关联 <span class="badge">{{ pagination.total }}</span></div>
          </div>
        </div>
    ...
    <a-modal v-model:open="modalVisible" :title="isEdit ? '编辑关联' : '新增关联'" width="520px" :destroy-on-close="true" @cancel="handleCancel" @ok="handleSubmit" :confirm-loading="submitLoading">
      <a-form ref="formRef" :model="formData" layout="vertical" :rules="rules">
        <a-row :gutter="16">
          ...
          <a-col :span="12"><a-form-item label="标签ID" name="tagId"><a-input-number v-model:value="formData.tagId" :min="0" style="width:100%" /></a-form-item></a-col>
        </a-row>
      </a-form>
    </a-modal>
  </div>
...
import type { ContentTagRelVo, ContentTagRelQuery, ContentTagRelCreate, ContentTagRelUpdate } from '@/api/content/contenttagrel'
import { getContentTagRelPage, getContentTagRelById, createContentTagRel, updateContentTagRel, deleteContentTagRel, batchDeleteContentTagRel } from '@/api/content/contenttagrel'
import { getRequestId } from '@/utils/idUtils.ts'

const columns = [
  { title: 'ID', dataIndex: 'id', key: 'id', width: 90 },
  { title: '内容ID', dataIndex: 'contentId', key: 'contentId', width: 160 },
  { title: '标签ID', dataIndex: 'tagId', key: 'tagId', width: 160 },
  { title: '操作', key: 'action', width: 160, fixed: 'right' as const, align: 'center' as const },
]

const loading = ref(false)
const submitLoading = ref(false)
const dataSource = ref<ContentTagRelVo[]>([])
const selectedRowKeys = ref<number[]>([])

const queryParams = reactive<ContentTagRelQuery>({
  page: 1,
  pageSize: 10,
})
...
type ContentTagRelFormData = ContentTagRelCreate & { id?: number }
const formData = reactive<ContentTagRelFormData>({
  id: undefined,
  contentId: undefined,
  tagId: undefined,
})
...
const loadData = async () => {
  loading.value = true
  try {
    const params: ContentTagRelQuery = { ...queryParams, page: pagination.current - 1, pageSize: pagination.pageSize }
    const res: any = await getContentTagRelPage(params)
    if (res.code === 200) {
...
const handleAdd = () => {
  isEdit.value = false
  Object.assign(formData, { id: undefined, contentId: undefined, tagId: undefined })
  modalVisible.value = true
}

const handleEdit = async (record: ContentTagRelVo) => {
  isEdit.value = true
  const res: any = await getContentTagRelById(record.id!)
  if (res.code === 200) {
...
const handleSubmit = async () => {
  try {
    ...
    const res: any = isEdit.value
      ? await updateContentTagRel(payload as ContentTagRelUpdate, reqId)
      : await createContentTagRel(payload, reqId)
    if (res.code === 200) {
...
const handleDeleteOne = (record: ContentTagRelVo) => {
  Modal.confirm({
    title: '确认删除',
    ...
    centered: true,
    onOk: async () => {
      const res: any = await deleteContentTagRel(record.id!)
      if (res.code === 200) {
...
const handleDelete = () => {
  ...
  Modal.confirm({
    ...
    centered: true,
    onOk: async () => {
      const res: any = await batchDeleteContentTagRel(selectedRowKeys.value)
      if (res.code === 200) {
        message.success('删除成功')
