    ...
    <div class="filter-bar">
      <a-form :model="queryParams" class="elegant-form">
        <div class="filter-grid">
          ...
          <div class="filter-item">
            <span class="filter-label">成长值</span>
            <a-input 
      ...
      <div class="board-toolbar">
        <div class="toolbar-left">
          <div class="board-tabs">
            <div class="tab active">用户等级账户 <span class="badge">{{ pagination.total }}</span></div>
          </div>
        </div>
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'avatar'">
              <div v-if="record.avatar" class="avatar-img-box">
                <img :src="record.avatar" class="avatar-img" />
              </div>
              <span v-else>--</span>
            </template>
            <!-- 个性头像框列 -->
            <template v-else-if="column.key === 'avatarFrame'">
              <div v-if="record.avatarFrame" class="avatar-img-box">
                <img :src="record.avatarFrame" class="avatar-img" />
              </div>
              <span v-else>--</span>
            </template>
            <!-- 操作列 -->
          ...
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item name="growthValue">
                <template #label><span class="form-label">成长值</span></template>
            ...
            <a-col :span="12">
              <a-form-item name="avatar">
                <template #label><span class="form-label">个性图标</span></template>
                <ImageUpload v-model="formData.avatar" value-type="id" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item name="avatarFrame">
                <template #label><span class="form-label">个性头像框</span></template>
                <ImageUpload v-model="formData.avatarFrame" value-type="id" />
              </a-form-item>
            </a-col>
          </a-row>
        </a-form>
...
import type { UserLevelAccountVo, UserLevelAccountQuery, UserLevelAccountCreate, UserLevelAccountUpdate } from '@/api/userGrowth/userlevelaccount.ts'
import { getUserLevelAccountPage, createUserLevelAccount, updateUserLevelAccount, deleteUserLevelAccount, batchDeleteUserLevelAccount, getUserLevelAccountById } from '@/api/userGrowth/userlevelaccount.ts'
...
const loadButtonPermissions = async () => {
  ...
  canAdd.value = hasButtonPermission('usergrowth:userlevelaccount:add')
  canUpdate.value = hasButtonPermission('usergrowth:userlevelaccount:update')
  canDelete.value = hasButtonPermission('usergrowth:userlevelaccount:delete')
}
...
const columns = [
  ...
  { title: '成长值', dataIndex: 'growthValue', key: 'growthValue', width: 120 },
  { title: '状态', dataIndex: 'status', key: 'status', width: 120 },
  { title: '个性图标', dataIndex: 'avatar', key: 'avatar', width: 100, align: 'center' },
  { title: '个性头像框', dataIndex: 'avatarFrame', key: 'avatarFrame', width: 120, align: 'center' },
...
const dataSource = ref<UserLevelAccountVo[]>([])
const selectedRowKeys = ref<number[]>([])

const queryParams = reactive<UserLevelAccountQuery>({
  page: 1,
  pageSize: 10,
  userId: undefined,
...
type FormData = UserLevelAccountCreate & { id?: number }
...
const loadData = async () => {
  ...
  try {
    ...
    const res = await getUserLevelAccountPage(params)
    if (res.code === 200) {
...
const handleStatusChange = async (record: UserLevelAccountVo) => {
  const newStatus = record.status === 1 ? 2 : 1
  requestId.value = getRequestId();
  const res = await updateUserLevelAccount({ ...record, status: newStatus } as UserLevelAccountUpdate, requestId.value)
  if (res.code === 200) {
...
const handleEdit = async (record: UserLevelAccountVo) => {
  isEdit.value = true
  try {
    const res = await getUserLevelAccountById(record.id!)
    if (res.code === 200) {
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) {
      const res = await updateUserLevelAccount(formData as UserLevelAccountUpdate, requestId.value)
      if (res.code === 200) {
      ...
      const submitData: UserLevelAccountCreate = { ...(formData as any) }
      delete (submitData as any).id
      const res = await createUserLevelAccount(submitData, requestId.value)
      if (res.code === 200) {
...
const handleDeleteOne = (record: UserLevelAccountVo) => {
  Modal.confirm({
    title: '危险操作确认',
    ...
    class: 'danger-modal',
    onOk: async () => {
      const res = await deleteUserLevelAccount(record.id!)
      if (res.code === 200) {
...
const handleDelete = () => {
  ...
  Modal.confirm({
    ...
    class: 'danger-modal',
    onOk: async () => {
      const res = await batchDeleteUserLevelAccount(selectedRowKeys.value)
      if (res.code === 200) {
