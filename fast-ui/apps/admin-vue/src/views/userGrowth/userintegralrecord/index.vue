    ...
    <div class="filter-bar">
      <a-form :model="queryParams" class="elegant-form">
        <div class="filter-grid">
          ...
          <div class="filter-item">
            <span class="filter-label">交易前积分</span>
            <a-input 
          ...
          <div class="filter-item">
            <span class="filter-label">交易后积分</span>
            <a-input 
      ...
      <div class="board-toolbar">
        <div class="toolbar-left">
          <div class="board-tabs">
            <div class="tab active">用户积分记录 <span class="badge">{{ pagination.total }}</span></div>
          </div>
        </div>
...
import type { UserIntegralRecordVo, UserIntegralRecordQuery, UserIntegralRecordCreate, UserIntegralRecordUpdate } from '@/api/userGrowth/userintegralrecord.ts'
import { getUserIntegralRecordPage, createUserIntegralRecord, updateUserIntegralRecord, deleteUserIntegralRecord, batchDeleteUserIntegralRecord, getUserIntegralRecordById } from '@/api/userGrowth/userintegralrecord.ts'
...
const loadButtonPermissions = async () => {
  ...
  canAdd.value = hasButtonPermission('usergrowth:userintegralrecord:add')
  canUpdate.value = hasButtonPermission('usergrowth:userintegralrecord:update')
  canDelete.value = hasButtonPermission('usergrowth:userintegralrecord:delete')
}

// 表格列定义
const columns = [
  { title: '用户ID', dataIndex: 'userId', key: 'userId' },
  { title: '交易前积分', dataIndex: 'beforeGrowthValue', key: 'beforeGrowthValue' },
  { title: '交易后积分', dataIndex: 'afterGrowthValue', key: 'afterGrowthValue' },
  { title: '变更积分', dataIndex: 'changeValue', key: 'changeValue' },
...
const dataSource = ref<UserIntegralRecordVo[]>([])
const selectedRowKeys = ref<number[]>([])

const queryParams = reactive<UserIntegralRecordQuery>({
  page: 1,
  pageSize: 10,
  userId: undefined,
...
type FormData = UserIntegralRecordCreate & { id?: number }
...
const loadData = async () => {
  ...
  try {
    ...
    const res = await getUserIntegralRecordPage(params)
    if (res.code === 200) {
...
const handleEdit = async (record: UserIntegralRecordVo) => {
  isEdit.value = true
  try {
    const res = await getUserIntegralRecordById(record.id!)
    if (res.code === 200) {
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) {
      const res = await updateUserIntegralRecord(formData as UserIntegralRecordUpdate, requestId.value)
      if (res.code === 200) {
      ...
      const submitData: UserIntegralRecordCreate = { ...(formData as any) }
      delete (submitData as any).id
      const res = await createUserIntegralRecord(submitData, requestId.value)
      if (res.code === 200) {
...
const handleDeleteOne = (record: UserIntegralRecordVo) => {
  Modal.confirm({
    title: '危险操作确认',
    ...
    class: 'danger-modal',
    onOk: async () => {
      const res = await deleteUserIntegralRecord(record.id!)
      if (res.code === 200) {
...
const handleDelete = () => {
  ...
  Modal.confirm({
    ...
    class: 'danger-modal',
    onOk: async () => {
      const res = await batchDeleteUserIntegralRecord(selectedRowKeys.value)
      if (res.code === 200) {
