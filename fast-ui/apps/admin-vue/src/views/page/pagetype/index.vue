          ...
          <a-row :gutter="24">
            <a-col :span="12">
              <a-form-item name="title">
                ...
                <a-input v-model:value="formData.title" placeholder="如：产品页面" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item name="code">
                <template #label><span class="form-label">类型编码</span></template>
                <a-input v-model:value="formData.code" placeholder="如：product" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item name="status">
                <template #label><span class="form-label">类型状态</span></template>
                <StatusCardSelect 
...
import {
  ...
  EditOutlined, TagOutlined, KeyOutlined, CheckCircleFilled, 
  MinusCircleFilled, LoadingOutlined, PlusCircleOutlined
} from '@ant-design/icons-vue'
import type { FormInstance } from 'ant-design-vue'
import type { PageTypeVo, PageTypeQuery, PageTypeCreate, PageTypeUpdate } from '@/api/page/pagetype.ts'
import { getPageTypePage, createPageType, updatePageType, deletePageType, batchDeletePageType, getPageTypeById } from '@/api/page/pagetype.ts'
...
const loadButtonPermissions = async () => {
  ...
  canAdd.value = hasButtonPermission('admin:page:type:add')
  canUpdate.value = hasButtonPermission('admin:page:type:update')
  canDelete.value = hasButtonPermission('admin:page:type:delete')
}
...
// 表格列定义
const columns = [
  { title: '类型标题', dataIndex: 'title', key: 'title', width: 180 },
...
const dataSource = ref<PageTypeVo[]>([])
const selectedRowKeys = ref<number[]>([])

const queryParams = reactive<PageTypeQuery>({
  page: 1,
  pageSize: 10,
  title: '',
  code: '',
...
type PageTypeFormData = PageTypeCreate & { id?: number }
const formData = reactive<PageTypeFormData>({
  id: undefined,
  title: '',
  code: '',
  status: 0,
...
const loadData = async () => {
  ...
  try {
    ...
    const res = await getPageTypePage(params)
    if (res.code === 200) {
...
const handleEdit = async (record: PageTypeVo) => {
  isEdit.value = true
  try {
    const res = await getPageTypeById(record.id!)
    if (res.code === 200) {
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) {
      const res = await updatePageType(formData as PageTypeUpdate, requestId.value)
      if (res.code === 200) {
      ...
      const submitData: PageTypeCreate = { ...(formData as any) }
      delete (submitData as any).id
      const res = await createPageType(submitData, requestId.value)
      if (res.code === 200) {
...
const handleDeleteOne = (record: PageTypeVo) => {
  Modal.confirm({
    title: '危险操作确认',
    ...
    class: 'danger-modal',
    onOk: async () => {
      const res = await deletePageType(record.id!)
      if (res.code === 200) {
...
const handleDelete = () => {
  ...
  Modal.confirm({
    ...
    class: 'danger-modal',
    onOk: async () => {
      const res = await batchDeletePageType(selectedRowKeys.value)
      if (res.code === 200) {
...
const handleStatusChange = async (record: PageTypeVo) => {
  const newStatus = record.status === 1 ? 2 : 1
  requestId.value = getRequestId();
  const res = await updatePageType({ ...record, status: newStatus } as PageTypeUpdate, requestId.value)
  if (res.code === 200) {
